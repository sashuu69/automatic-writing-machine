...
path = sys.argv[1]
fileMidName = os.path.basename(path)
fileName = os.path.splitext(fileMidName)[0]
...
###########################
def grayscale(rgb):
    # 0.299,0.587,0.114  0.109,0.587,0.254
    return np.dot(rgb[..., :3], [0.259, 0.587, 0.114])
    # 0.199       0.001
...
ret, mask = cv2.threshold(
    sketch, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
...
# save image
cv2.imwrite(home + "/" + fileName + ".jpg", invertedImage)

# Wait for 1 seconds
time.sleep(1)

# generate gcode from rastercarve
gcode = os.popen("rastercarve --width 10 " + home +
                 "/" + fileName + ".jpg &> /dev/null").read()

gcodeOutput = home + "/" + fileName + ".gcode"
# save gcode file to dataURI
f = open(gcodeOutput, 'w')
